...
function Download-File([string]$Url, [string]$Destination, [hashtable]$Headers) {
    ...
    & $curl.Source -L --fail --retry 3 --retry-delay 2 --connect-timeout 20 `
        -A 'CFW-Meta-Core-Patch-Updater/1.5.3' -o $Destination $Url
...
try {
    ...
    $headers = @{
        'User-Agent' = 'CFW-Meta-Core-Patch-Updater/1.5.3'
    ...
    @(
        ...
        'Updater: Clash for Windows Meta Core Patch v1.5.3 official-release updater',
